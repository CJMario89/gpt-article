...
export const getNearPlaces = async ({
  ...
  const query = {
      ...
      FROM 
        (
          ...
          FROM 
            SpotInfo
          WHERE 
            SpotInfo.articleType = 'restuarant' 
          ORDER BY distance 
          LIMIT ${limit} 
          OFFSET ${isRestuarant ? (page - 1) * limit + 1 : (page - 1) * limit}
        ) 
      AS SpotInfo
      JOIN SpotImage ON 
        SpotImage.id = (
          ...
          FROM 
            SpotImage 
          WHERE 
            SpotImage.city = SpotInfo.city AND 
    ...
    spot: `
      ...
      FROM (
        ...
        FROM 
          SpotInfo 
        WHERE 
          SpotInfo.articleType = 'spot' 
        ORDER BY distance 
        LIMIT ${limit} 
        OFFSET ${isSpot ? (page - 1) * limit + 1 : (page - 1) * limit}
      ) 
      AS SpotInfo
      JOIN SpotImage ON 
        SpotImage.id = (
    ...
    prefecture: `
      ...
      AS PrefectureInfo
      JOIN PrefectureImage ON 
        PrefectureImage.id = (
          SELECT 
            MIN(id) 
          FROM 
            PrefectureImage 
          WHERE PrefectureImage.prefecture = PrefectureInfo.prefecture
        )`,
  };
